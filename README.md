...
بدل أن يفتح المستخدم التطبيق ليكتشف وجود طلب مساعدة جديد أو رسالة، يصله إشعار فوري على شاشة الهاتف — حتى لو كان التطبيق مغلقاً تماماً.

### الملفات المطلوبة (يجب توفيرها قبل البناء)

| الملف | من أين | أين يوضع |
|---|---|---|
| `google-services.json` | Firebase Console → Project Settings → أضف تطبيق أندرويد باسم الحزمة `com.socialfund` → نزّل الملف | `android/app/google-services.json` |
| — | — | (تأكد أن `android/build.gradle` يحتوي على `classpath 'com.google.gms:google-services:...'`، وأن `android/app/build.gradle` ينتهي بسطر `apply plugin: "com.google.gms.google-services"` — كلاهما مُضاف مسبقاً في هذا المشروع) |

إن غيّرت اسم الحزمة (راجع قسم "تغيير اسم الحزمة" أدناه)، يجب توليد `google-services.json` جديد من Firebase مطابق للاسم الجديد، وإلا فشلت المزامنة مع مشروع Firebase.

### كيف تعمل

1. عند تسجيل الدخول بنجاح، يطلب التطبيق تلقائياً إذن الإشعارات من المستخدم (مطلوب إجبارياً على أندرويد 13+)
2. عند الموافقة، يحصل التطبيق على "رمز جهاز" فريد من Firebase ويرسله للخادم (`POST /push/register-token`)
3. الخادم (`SocialFundBackend`) يستخدم هذا الرمز لاحقاً لإرسال إشعارات حقيقية عبر Firebase Admin SDK — راجع قسم "الإشعارات الفورية" في `SocialFundBackend/README.md` لإعداد جانب الخادم (يتطلب ملف اعتماد Firebase منفصل)
4. عند تسجيل الخروج، يُلغى تسجيل الجهاز فلا يستمر باستقبال إشعارات لحساب لم يعد مسجّلاً به

### سلوك الإشعار حسب حالة التطبيق

| حالة التطبيق | السلوك |
|---|---|
| مفتوح (Foreground) | يظهر كإشعار داخلي (Toast) أعلى الشاشة بدل إشعار نظام، لأن المستخدم أصلاً ينظر للشاشة |
| بالخلفية أو مغلق | إشعار نظام أندرويد عادي بأيقونة وألوان الصندوق؛ الضغط عليه يفتح التطبيق وينتقل مباشرة للشاشة المناسبة (مثال: إشعار "طلب مساعدة جديد" يفتح شاشة المساعدات مباشرة) |
